...
| 処理 | ファイル | スケジュール(JST) |
|---|---|---|
| Phase 0 初期化(D1履歴400本×全監視ペア) | `scripts/init-history.js` | 手動1回(Actions → Init History) |
| 日次バッチ(指標計算・フィード生成) | `scripts/daily.js` | 火〜土 7:20 |
| 日中バッチ(急変検知・イベント窓再判定) | `scripts/intraday.js` | 1時間8回(市場時間のみ・既存FXDaily-Levelsと分ずらし) |
...
| フラグ | 条件 | 意味 |
|---|---|---|
| `no_new_grid` | レジーム高ボラ以上 or イベント窓内 | 新規グリッド開始・段数追加の禁止(**全12ペアで算出**) |
| `halt_all_new` | レジーム異常 or 急変フラグ | 新規全停止(+運用監視を日次モードへ)(**全12ペアで算出**) |
| `vr_hold_check` | 金曜15:00JST以降・土曜 or 日銀窓(前30h/後2h) | USDJPYのみ。ゲートフラグに**併記**される週次ルール(週末・日銀またぎ保有確認)のリマインダー。停止指示ではない |

2026-07-31変更: USDJPY・XAUUSDもゲートフラグの算出対象とした(従来はUSDJPY=保有確認のみ/XAUUSD=参考算出+`monitor_only`マーカー)。旧プロファイル(`hold_flag_only`/`monitor_only`)はコード上は後方互換のため残るが現在未使用。
...
1. GitHub Actions の scheduled cron は仕様上5分間隔が最短(公式ドキュメント)。急変フラグは1時間8回(市場時間のみ)の間隔判定であり、実行自体も数分程度遅延し得るためリアルタイム検知ではない(ティックレベル即応が必要な場合はPhase 2でEA/VPS側の補完を検討)
